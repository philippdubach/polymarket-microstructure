...
\subsection{The CTF Exchange contract}
...
executes settlement on chain. Two contract versions are in scope.
V1 (\texttt{0x4bFb41d5\allowbreak B3570DeFd03\allowbreak C39a9A4D8dE6Bd8\allowbreak B8982E})
ran from launch through the end of April 2026 and is the contract our
28-day scrape window targets. V2
(\texttt{0xE111180000d2663C\allowbreak 0091e4f400237545\allowbreak B87B996B})
launched in early April 2026 with a hard cutover; V1 orders are
...
\texttt{(orderHash, maker, taker, makerAssetId, takerAssetId,
makerAmountFilled, takerAmountFilled, fee)}. The asset ids identify
...
\subsection{The public WebSocket feed}
...
distinction is the proximate cause of the measurement gap we
document in Section~\ref{sec:limits}: an inference algorithm working
from the feed alone cannot reliably reconstruct the aggressor sign
because the underlying field never encodes it.

\subsection{Why aggressor sign matters}
...
component of Glosten-Harris all instantiate this form. A noisy
$\mathrm{sign}_t$ does not just attenuate $M$; depending on the
distribution of $f$ across trades, it can flip the sign of $M$.
This is why the 59\% sign-agreement rate documented in
Section~\ref{ssec:sign-agreement} translates into the 67\% effective-
spread and 60\% Kyle's $\lambda$ sign-flip rates we report on the
top-100 panel.

\subsection{Comparison to equity-market microstructure}\label{ssec:equity-comparison}

Several of our findings only acquire scale once they are placed
against the equity-market benchmarks. \citet{ellis-michaely-ohara-2000}
report that the Lee-Ready algorithm correctly classifies 81\% of
trades on Nasdaq, and the original \citet{lee-ready-1991} study
reports a similar accuracy on NYSE TAQ; the failure of feed-driven
inference on Polymarket at $\sim 59\%$ is therefore around 22
percentage points below the equity benchmark on the same algorithm.
...
venues \citet{cong-li-niu-2023} document wash-trade shares of
25--70\%; Polymarket sits at a median 1\% under our lower-bound
detector with a 22\% upper tail, between the two regimes. Finally,
