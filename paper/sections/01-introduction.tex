...
Microstructure determines the cost a participant pays per
informational position and bounds how much the venue can aggregate;
if a prediction market's microstructure is noisy, prices are noisier
than the headline-level aggregation literature implicitly assumes.
...
The empirical core of the paper is two findings, ordered here by
which carries more weight for the literature.

\textit{Trade-direction inference from Polymarket's public feed is
noise-dominated.} Direction-dependent microstructure measures
(effective spread, realized spread, Kyle's $\lambda$, Amihud,
Roll, Abdi-Ranaldo) need an aggressor sign for each trade, and
empirical practice on equity venues sources that sign from a
quote-driven feed via Lee-Ready or its variants
\citep{lee-ready-1991,ellis-michaely-ohara-2000}. Polymarket's public
feed does not expose enough information to do this reliably: the
\texttt{change\_side} field marks which side of the book moved, not
which side initiated the trade. We document the consequences. Sign
agreement between feed-inferred and on-chain trade directions sits at
$\sim 59\%$ volume-weighted across the top-100 stratum and four
disjoint 7-day windows, just above the 50\% chance baseline. Within
the comparable subset of the top-100 panel this propagates to a 67\%
sign-flip rate on the effective half-spread and a 60\% sign-flip rate
on Kyle's $\lambda$ between feed-inferred and on-chain estimates. Any
microstructure result on Polymarket that depends on trade direction
therefore needs to source that direction from on-chain
\texttt{OrderFilled} events. We release a replication package that
performs the join.
...
These two findings are complementary. The eight stylized facts
characterise Polymarket's microstructure on its own terms; the
methodological result establishes the conditions under which any
trade-direction-dependent statement about Polymarket can be trusted.

The most surprising finding by some distance is the measurement
result of Section~\ref{sec:limits}; readers focused on the
empirical-asset side of the contribution will likely care most about
SF1 (longshot premium), SF2 (depth concentration), and SF8 (depth
decay near resolution).
