...
literature \citep{snowberg-wolfers-2010,thaler-ziemba-1988}.
...
The panel-level distribution is concentrated around the null: median
...
rejects in \textbf{351 of 600 markets} (58.5\%) at $\alpha=0.05$~--~a
mechanical consequence of high event counts (millions of events make
the binomial test sensitive to tiny deviations from 0.10). The
economic-magnitude interpretation is that quote-timing does
\emph{not} cluster on block boundaries to a meaningful degree, even
though high-volume markets statistically reject the exact point null.
...
on cryptocurrency venues (where wash shares of 25--70\% have been
documented for unregulated exchanges).

Across 600 markets and 6.4\,M trades, the median wash share is
\textbf{0.97\%}, with $p_{90}=4.5\%$, $p_{99}=10.6\%$, and a maximum
of 22.2\%. No market in the panel reaches the lower end of the
crypto-venue range. The gap between our lower bound and the
network-based literature represents wash activity detectable only via
multi-counterparty graph analysis.
...
\begin{table}[htbp]
  ...
  \scriptsize
  \input{tables/sf8_depth_decay}
\end{table}

The bivariate slope is \textbf{0.818} (HC3 SE 0.113, $t=7.2$,
$R^2 = 0.13$). Adding category fixed effects (Crypto, Sports, Other,
Geopolitics) attenuates the slope to \textbf{0.550} (HC3 SE 0.143,
$t=3.85$, $R^2=0.22$); roughly a third of the bivariate
depth-decay association is category-level confounding. Adding log
panel-window volume on top of category attenuates the slope further
to \textbf{0.305} (HC3 SE 0.104, $t=2.94$, $R^2=0.49$). Volume
absorbs much of the cross-sectional depth variation, but the
time-to-close coefficient remains positive and significant. The
$0.31$ slope corresponds to $\sim 6\%$ less mean depth per
$10\times$ reduction in seconds-to-close, a smaller economic
magnitude than either the bivariate or the category-FE specification
suggested.
